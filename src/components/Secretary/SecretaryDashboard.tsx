...
const SecretaryDashboard = () => {
  ...
  useEffect(() => {
    ...
    interface Assignment { secretaryId: string; status: string; chatId: string; }
    const myActiveChats = assignments
      .filter((a: Assignment) => a.secretaryId === user?.id && a.status === 'active')
  ...
  const sendMessage = () => {
    ...
    sendAdminMessage(selectedChat, newMessage, user?.name || 'Secretaria');
    setNewMessage('');
  };
  ...
  const completeChat = (chatId: string) => {
    ...
    const updatedAssignments = assignments.map((a: Assignment) => 
      a.chatId === chatId && a.secretaryId === user?.id && a.status === 'active'
        ? { ...a, status: 'completed' }
        : a
    );
    ...
    sendAdminMessage(chatId, '¡Gracias por contactar a Soltice Energy! Ha sido un placer ayudarte. ¡Que tengas un excelente día! 🌞⚡', user?.name || 'Secretaria');
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-black text-white">Panel de Secretaria</h1>
